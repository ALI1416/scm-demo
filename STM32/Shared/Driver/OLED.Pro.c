 ...
 */
void OLED_ShowFixedNumber(uint8_t X, uint8_t Y, uint32_t Number, uint8_t Length)
{
  if (Length == 0)
  {
    return;
 ...
 * @param  Length 长度(1-16)
 */
void OLED_ShowFixedSignedNumber(uint8_t X, uint8_t Y, int32_t Number, uint8_t Length)
{
  uint32_t temp;
  if (Number < 0)
  {
  ...
  OLED_ShowFixedNumber(X + 8, Y, temp, Length - 1);
}
