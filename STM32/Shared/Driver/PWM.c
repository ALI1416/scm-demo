...
void PWM_Init_PA0(uint16_t TIM_Prescaler, uint16_t TIM_Period, uint16_t TIM_Pulse)
{
  // 初始化TIM2
  // 在stm32f10x_tim文件里
  // TIM_OC1Init 初始化输出比较(Output Compare)通道
  // TIM_OCStructInit 给输出比较模块结构体默认值
  // TIM_CtrlPWMOutputs 高级定时器PWM输出使能
  // TIM_ForcedOC1Config 配置强制输出模式
  // TIM_OC1PreloadConfig 配置CCR寄存器预装功能
  // TIM_OC1FastConfig 配置快速使能
  // TIM_ClearOC1Ref 外部事件时清除REF信号
  // TIM_OC1PolarityConfig 设置定时器输出比较极性
  // TIM_OC1NPolarityConfig 高级定时器互补通道配置
  // TIM_CCxCmd 和 TIM_CCxNCmd 修改输出使能
  // TIM_SelectOCxM 选择输出比较模式
  // TIM_SetCompare1 修改CCR寄存器值
  ...
  // TIM_OCMode_Active 高电平
  // TIM_OCMode_Inactive 低电平
  // TIM_OCMode_Toggle 电平反转
  // TIM_OCMode_PWM1 PWM模式1
  // TIM_OCMode_PWM2 PWM模式2
  // TIM_ForcedAction_Active 强制高电平输出
  // TIM_ForcedAction_InActive 强制低电平输出
  TIM_OCInitStructure.TIM_OCMode = TIM_OCMode_PWM1;
  // 极性
  // TIM_OCPolarity_High 正常
  // TIM_OCPolarity_Low 翻转
...
void PWM_Init_PA15(uint16_t TIM_Prescaler, uint16_t TIM_Period, uint16_t TIM_Pulse)
{
  ...
  // GPIO_Remap_SWJ_NoJTRST 解除NJTRST调试(PB4)
  // GPIO_Remap_SWJ_JTAGDisable 解除JTAG调试(PA15、PB3、PB4)
  // GPIO_Remap_SWJ_Disable 解除SWD和JTAG调试(PA13、PA14、PA15、PB3、PB4) 谨慎使用，否则STLINK无法下载重新
